...
- Keep the Android application version in the Gradle configuration as the source of truth until a dedicated update mechanism is introduced.
- If a future update mechanism adds `app/src/main/assets/update-version.properties`, change the `updateVersionName` value there for update metadata.
- Do not change an update version directly in application logic.
